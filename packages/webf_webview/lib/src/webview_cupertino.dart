...
class CupertinoWebView implements WebViewPlatform {
  ...
  Future<UiKitViewController> getUiKitViewController(
      int id, CreationParams creationParams) async {
    return PlatformViewsService.initUiKitView(
      id: id,
      viewType: 'plugins.webf/webview',
  ...
  RenderBox buildRenderBox({
    ...
    getUiKitViewController(_id!, creationParams!)
      ...
      MethodChannelWebViewPlatform methodChannelWebViewPlatform =
          MethodChannelWebViewPlatform(_id, webViewPlatformCallbacksHandler!);
  ...
  void _polyfillWebView(
      MethodChannelWebViewPlatform methodChannelWebViewPlatform) {
    String invoker = '''
      // Polyfill webf env for iOS.
      ;(function(){
        var webf = window.webf = window.webf || {};
        webf.postMessage = function(message) {
          return webkit.messageHandlers.webf.postMessage(message);
        };
      })();
    ''';
