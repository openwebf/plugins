...
- (instancetype)initWithFrame:(CGRect)frame
  ...
  if (self = [super init]) {
    ...
    NSString* channelName = [NSString stringWithFormat:@"plugins.webf/webview_%lld", viewId];
...
- (void)setupJavascriptBridge:(FlutterResult)result {
  [_webView.configuration.userContentController addScriptMessageHandler:self name:@"webf"];
  result(nil);
}

-(void)teardownJavascriptBridge:(FlutterResult)result {
  [_webView.configuration.userContentController removeScriptMessageHandlerForName:@"webf"];
  result(nil);
}
...
{
  if ([message.name isEqualToString:@"webf"]) {
